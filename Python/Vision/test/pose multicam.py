...
weight=(2)
rColor=(0,0,255)
#dualcam=zip(picam2.capture_array(),webcam.capture_array())
...
with mp_pose.Pose(min_detection_confidence=.5,min_tracking_confidence=.5) as pose:
    ...
    #        mpDraw.draw_landmarks(frame, results.pose_landmarks, mp.solutions.pose.POSE_CONNECTIONS)
        try:
            ...
            wlandmarks=wresults.pose_landmarks.landmark
            #print(landmarks)
...
#Muay Thai
    ...
    #    cv2.putText(frame,'Right Uppercut:'+str(rUcutcounter),(280,40),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,0,255),1,cv2.LINE_AA)
        ...
        cv2.putText(frame2,"Stance: " + str(stance),(10,60),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,0,255),1,cv2.LINE_AA)
        ...
        cv2.imshow("picam", frame)        cv2.putText(frame,'Left Kick:'+str(lKickcounter)+ ' Vel: '+str(lAnlkeVel),(400,20),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,0,255),1,cv2.LINE_AA)
